...
curl -s "$BASE_URL/api/v1/strategies?status=enabled" | jq .
echo ""

# 4. Get Disabled Strategies
echo "4. Get Disabled Strategies..."
curl -s "$BASE_URL/api/v1/strategies?status=disabled" | jq .
echo ""

# 5. Create Test Strategy
echo "5. Create Test Strategy..."
STRATEGY_RESPONSE=$(curl -s -X POST "$BASE_URL/api/v1/strategies" \
...
curl -s -X POST "$BASE_URL/api/v1/strategies/$STRATEGY_ID/enable" | jq .
echo ""
...
curl -s -X POST "$BASE_URL/api/v1/strategies/$STRATEGY_ID/disable" | jq .
echo ""
...
curl -s "$BASE_URL/api/v1/strategies?status=true" | jq '.total'
